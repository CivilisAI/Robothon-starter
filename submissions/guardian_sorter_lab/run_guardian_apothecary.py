...
def overlay(frame: np.ndarray, state: dict) -> np.ndarray:
    ...
    if state["phase"] == "in_hand_cap_rotation":
        ...
        draw.text((cx - 58, cy + 64), "cap angle", fill=(238, 246, 255), font=body_font)
...
def write_artifacts(dataset_dir: Path, observations: list[dict], model: mujoco.MjModel, metrics: dict, weights: dict) -> None:
    ...
    challenge_evidence = {
        ...
        "judge_front_matter": [
            "The keyframe storyboard summarizes the full 20-second supercut video in eight readable panels.",
    ...
    captions = [
        (0, 1, "Scan vial, cap, sterile pod, blister pack, syringe, and dose dial."),
        (1, 3, "Visual-servo approach uses the learned tactile residual policy."),
        (3, 5, "Five fingers close and tactile contacts stabilize the vial."),
        (5, 9, "In-hand cap rotation exceeds 200 degrees without losing the vial."),
        (9, 11, "The hold survives a 4N lateral shove and a 9x load challenge."),
        (11, 13, "A lateral slip disturbance is recovered under 1.2 millimeters."),
        (13, 15, "The vial is delivered to the sterile pod and verified."),
        (15, 16, "The audit button is pressed after delivery."),
        (16, 17, "A blister pill is pressed from its pack."),
        (17, 18, "The syringe plunger is dosed with two-finger control."),
        (18, 19, "The dose dial is turned to confirm the care sequence."),
        (19, 20, "Metrics, stress replay, and policy card are exported."),
    ]
    srt = []
...
def run_episode(
    *,
    ...
    keyframe_targets = {
        ...
        int(0.34 * (total_frames - 1)): "03_cap",
        int(0.45 * (total_frames - 1)): "04_shove",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sample-hz", type=int, default=12)
