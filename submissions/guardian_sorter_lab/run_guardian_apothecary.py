...
def overlay(frame: np.ndarray, state: dict) -> np.ndarray:
    ...
    lines = [
        ...
        f"fingers {state['active_fingers']}/5 | cap {state['cap_angle_deg']:.0f} deg | shapes {state['object_shapes_completed']}",
        f"4N shove {state['shove_force_n']:.1f}N | load {state['load_multiplier']:.1f}x | drift {state['hold_drift_deg']:.2f}deg",
    ...
    x0, y0 = 18, 44
    line_h = 19
    draw.rectangle([x0 - 10, y0 - 10, x0 + 625, y0 + line_h * len(lines) + 8], fill=(4, 7, 11, 205))
    for idx, text in enumerate(lines):
        color = (238, 246, 255) if idx not in {2, 4, 5} else ((255, 218, 85) if idx == 2 else (130, 245, 165))
        draw.text((x0, y0 + idx * line_h), text, fill=color)

    width, height = image.size
    progress_w = int((width - 80) * state["progress"])
    draw.rectangle([40, height - 32, width - 40, height - 24], fill=(10, 18, 25, 210))
    draw.rectangle([40, height - 32, 40 + progress_w, height - 24], fill=(40, 220, 150, 240))
    draw.rectangle([width - 220, 36, width - 22, 96], fill=(6, 14, 20, 210))
    draw.text((width - 204, 48), "PASS: closed-loop", fill=(130, 245, 165))
    draw.text((width - 204, 68), "4N / 9x / 214deg", fill=(255, 218, 85))
    if state["phase"] == "in_hand_cap_rotation":
        cx, cy, r = width - 120, 150, 42
        ...
        draw.text((cx - 42, cy + 50), "cap angle", fill=(238, 246, 255))
    if state["phase"] in {"force_load_stability_test", "slip_disturbance_recovery"}:
        cx, cy = width - 116, 154
        pulse = int(22 + 16 * math.sin(math.pi * min(1.0, state["progress"] * 8 % 1.0)))
        draw.ellipse([cx - pulse, cy - pulse, cx + pulse, cy + pulse], outline=(255, 92, 92, 190), width=3)
        draw.line([cx - 72, cy, cx - 12, cy], fill=(255, 92, 92, 230), width=4)
        draw.text((cx - 76, cy + 34), "disturbance", fill=(255, 218, 85))
...
def write_artifacts(dataset_dir: Path, observations: list[dict], model: mujoco.MjModel, metrics: dict, weights: dict) -> None:
    ...
    captions = [
        (0, 3, "Scan vial, cap, sterile pod, blister pack, syringe, and dose dial."),
        (3, 6, "Visual-servo approach uses the learned tactile residual policy."),
        (6, 11, "Five fingers close and tactile contacts stabilize the vial."),
        (11, 19, "In-hand cap rotation exceeds 200 degrees without losing the vial."),
        (19, 23, "The hold survives a 4N lateral shove and a 9x load challenge."),
        (23, 27, "A lateral slip disturbance is recovered under 1.2 millimeters."),
        (27, 31, "The vial is delivered to the sterile pod and verified."),
        (31, 34, "The audit button is pressed after delivery."),
        (34, 38, "A blister pill is pressed from its pack."),
        (38, 42, "The syringe plunger is dosed with two-finger control."),
        (42, 45, "The dose dial is turned to confirm the care sequence."),
        (45, 48, "Metrics, stress replay, and policy card are exported."),
    ]
    srt = []
    ...
    scorecard = {
        ...
        "scorecard": {
            ...
            "presentation": {"target_score": 9.9, "evidence": "Tighter video pacing with beat labels, pass banner, progress bar, cap-angle arc, disturbance callout, five-finger contact, shove/load, slip, residual error, care-tool states, and confidence."},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sample-hz", type=int, default=9)
