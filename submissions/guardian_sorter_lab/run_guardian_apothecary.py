...
def overlay(frame: np.ndarray, state: dict) -> np.ndarray:
    ...
    headline_map = {
        ...
        "sterile_pod_delivery": "DELIVER TO POD",
        "audit_button_press": "DOSE VERIFY",
        "blister_pack_press": "DOSE VERIFY",
        "syringe_plunger_dose": "DOSE VERIFY",
        "dose_dial_confirm": "DOSE VERIFY",
    ...
    proof_map = {
        ...
        "sterile_pod_delivery": "transfer completes after the save",
        "audit_button_press": "button, blister, syringe, dial verified",
        "blister_pack_press": "button, blister, syringe, dial verified",
        "syringe_plunger_dose": "button, blister, syringe, dial verified",
        "dose_dial_confirm": "button, blister, syringe, dial verified",
        "final_report_export": "uncap, save, deliver, dose verified",
    }
    ...
    if state["progress"] < 0.055:
        ...
        badges = ["214 DEG CAP", "4MS REFLEX", "30/30 SKILLS", "96/96 STRESS"]
        badge_w = 150
        badge_gap = 12
    ...
    if state["progress"] > 0.94:
        ...
        draw.text((x0 + 24, y0 + 18), "DOSE READY", fill=(130, 245, 165), font=headline_font)
        draw.text((x0 + 24, y0 + 55), "uncap | save | deliver | dose", fill=(255, 218, 85), font=body_font)
        draw.text((x0 + 24, y0 + 82), "30/30 skills | 96/96 stress | 4ms reflex", fill=(238, 246, 255), font=body_font)
...
def write_keyframe_sheet(path: Path, keyframes: dict[str, np.ndarray]) -> None:
    labels = [
        ...
        ("06_delivery", "06 DELIVER", "sterile pod verified"),
        ("07_tools", "07 DOSE VERIFY", "button, blister, syringe, dial"),
        ("08_report", "08 RESCUE PASS", "uncap, save, deliver, dose"),
    ]
...
def write_artifacts(dataset_dir: Path, observations: list[dict], model: mujoco.MjModel, metrics: dict, weights: dict) -> None:
    ...
    challenge_evidence = {
        "project": metrics["project"],
        "uuid": UUID,
        "scoreboard_description": "Fragile-vial rescue: uncap, save, deliver, dose verify; 30/30 skills and 96/96 stress pass.",
        "one_sentence": "Vision+tactile fragile-vial rescue with five-finger grasp, 214 degree cap rotation, 4ms reflex latency, 4N/9x disturbance hold, sterile delivery, dose verification, 30/30 care-skill variants, 11/11 criteria, and 96/96 stress-rollout success.",
        "judge_front_matter": [
            "The demo is a clean 64-second rescue evidence-card run: one claim per phase, contact dots, opening badges, shove risk, grasp-saved recovery, sterile delivery, dose verification, and a closing dose-ready card.",
    ...
    captions = [
        (0, 8, "Fragile vial rescue: uncap, save, deliver, dose."),
        ...
        (38, 46, "A 4N shove tries to knock the vial loose."),
        (46, 52, "The 4ms tactile reflex catches the slip."),
        (52, 60, "Sterile delivery, button, blister, syringe, and dial verify."),
        (60, 64, "Dose ready: 30 skills, 96 stress rollouts."),
    ]
    srt = []
    ...
    scorecard = {
        ...
        "scorecard": {
            ...
            "task_design": {"target_score": 9.9, "evidence": "Medication disaster-triage challenge: scan, five-finger grasp, cap rotation, 4N shove, 9x load hold, slip recovery, pod delivery, audit button, blister press, syringe dosing, dose dial, and supporting clinic-transfer scenarios."},
            ...
            "presentation": {"target_score": 9.9, "evidence": "Clean 64-second rescue evidence-card video with one-line overlays, dot-based five-finger contact indicator, closer grasp/cap framing, uncap highlight marker, shove risk marker, grasp-saved recovery marker, delivery marker, dose-ready card, cap-angle arc, 4N/9x callout, readable keyframe storyboard, and concise SRT narration."},
...
cluttering the video. The visible story is deliberately simple: uncap, save,
deliver, dose verify.
...
1. `media/demo.mp4` - clean 64-second generated rescue demo with one-line evidence-card overlays, dot-based five-finger contact indicator, closer grasp/cap framing, uncap highlight marker, drop-risk marker, grasp-saved recovery marker, sterile-delivery marker, dose-ready card, cap-angle arc, 4N/9x callout, and opening evidence badges.
2. `media/keyframes.png` - eight-panel storyboard of scan, grasp, 214 deg uncap, 4N/9x hold, slip recovery, delivery, dose verification, and rescue pass.
...
- 56-87%: the saved vial transfers to the sterile pod and completes audit, blister, syringe, and dose-dial checks.
- 87-100%: dose-ready confirmation and evidence export close the benchmark with 30/30 skills, 11/11 criteria, and 96/96 stress pass.
...
- Presentation: clean 64-second rescue evidence-card video plus keyframe storyboard uses one-line overlays, contact dots, closer grasp/cap framing, uncap, drop-risk, grasp-saved, delivery, and dose-ready markers, opening evidence cards, cap-angle arc, 4N/9x callout, and concise SRT captions.
