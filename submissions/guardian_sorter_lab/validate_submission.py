...
def main() -> int:
    ...
    if summary.get("learned_policy_inference_samples", 0) < 550:
        raise SystemExit("Formal demo did not run long enough for paced evidence")
    if not (60.0 <= float(summary.get("formal_demo_duration_s", 0.0)) <= 180.0):
        raise SystemExit("Formal demo duration is outside the official 1-3 minute window")
    ...
    if summary.get("real_world_demo_count", 0) < 12:
    ...
    if skill_suite.get("variants", 0) < 30:
        raise SystemExit("Care-skill suite has too few variants")
    if skill_suite.get("passed", 0) < skill_suite.get("variants", 0):
        raise SystemExit("Care-skill suite did not pass all variants")
    if summary.get("care_skill_suite_passed", 0) < 30 or summary.get("care_skill_success_rate", 0.0) < 1.0:
        raise SystemExit("Care-skill suite metrics missing from closed-loop summary")
    if clinic_scenarios.get("scenarios", 0) < 12:
        raise SystemExit("Clinic-scenario suite has too few scenarios")
    if clinic_scenarios.get("passed", 0) < clinic_scenarios.get("scenarios", 0):
    ...
    skill_passed = skill_suite.get("passed", 0)
    skill_total = skill_suite.get("variants", 0)
    clinic_passed = clinic_scenarios.get("passed", 0)
    clinic_total = clinic_scenarios.get("scenarios", 0)
        ...
        json.dumps(
            {
                ...
                "formal_demo_duration_s": summary.get("formal_demo_duration_s"),
                ...
                "care_skill_suite_passed": skill_passed,
                "care_skill_suite_total": skill_total,
                "clinic_scenarios_passed": clinic_passed,
                "clinic_scenarios_total": clinic_total,
