...
18/18 care-skill pass, and 96/96 stress pass.
...
machine-readable evidence pack. The regenerated demo is back to a concise
60-second highlight run: it keeps the middle of the video deliberately clean
with one-line phase overlays, a dot-based five-finger contact indicator, closer
grasp/cap framing, an uncap completion marker, a recovered-slip marker, opening
evidence badges, and a closing pass card. The detailed technical evidence lives
in the README, JUDGE_BRIEF, metrics, contact timeline, and stress replay.
...
touch sensors, learned policy weights, contact timelines, an 18-variant skill
suite, a video-to-rubric map, stress replays, 500Hz control-loop evidence, and a
local validator.

Primary judge signal: 18/18 care-skill variants pass, 11/11 task criteria pass,
96/96 fixed-seed stress rollouts pass, 214.0 degree cap rotation, 4.0 ms tactile
reflex latency, 280 stable five-finger contact samples, and 0.35 mm recovered
slip.
...
|---|---|
| Reproducibility | `run_guardian_sorter.py` regenerates `scene.xml`, `media/demo.mp4`, `media/keyframes.png`, metrics, labels, contact timeline, narrative beat map, 18-variant skill suite, stress replay, policy card, and judge artifacts. `validate_submission.py` checks the UUID and all success thresholds. |
...
| Engineering quality | The submission includes a metrics schema, policy card, 18/18 skill-suite replay, stress replay, trajectory trace, contact timeline, label CSV, sensor manifest, narrative beat JSON, SRT captions, rubric scorecard, manifest, and a validator. |
| Presentation | `media/demo.mp4` is a concise 60-second highlight demo within the official 1-3 minute window. It uses one-line middle overlays, a dot-based five-finger contact indicator, opening/closing evidence cards, closer grasp/cap framing, an uncap completion marker, a recovered-slip marker, a cap-angle arc, and a 4N/9x disturbance callout. `media/keyframes.png` gives an eight-panel storyboard with concise subtitles, and `JUDGE_BRIEF.md` provides the short scoring entry point. |
...
- Five-finger stable contact: 280 samples with all five fingers active and
  balanced.
- Care-skill suite: 18/18 fixed-seed variants pass across grasp, uncap,
...
- In-hand cap rotation over 200 degrees, five active contacts, 4N lateral shove
  ...
  confirmation.
- Formal pass evidence: 18/18 care-skill variants, 11/11 task criteria, 96/96
  fixed-seed stress rollouts, 100% learned-policy stress success, 500Hz loop,
  and 4ms tactile reflex latency.
- Concise 60-second generated demo video with one-line overlays, contact dots,
...
6. `dataset/skill_suite_eval.json` - 18/18 care-skill variants.
7. `dataset/contact_timeline.json` - per-sample fingertip contact, balance, and
   slip evidence.
8. `dataset/stress_eval.json` - fixed-seed perturbation replay.
9. `learned_policy_weights.json` and `dataset/training_report.json` - learned
   policy provenance.
