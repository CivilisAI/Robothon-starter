...
tools, and export a machine-readable evidence pack.
...
touch sensors, learned policy weights, contact timelines, stress replays, and a
local validator.
...
|---|---|
| Reproducibility | `run_guardian_sorter.py` regenerates `scene.xml`, `media/demo.mp4`, `media/keyframes.png`, metrics, labels, contact timeline, stress replay, policy card, and judge artifacts. `validate_submission.py` checks the UUID and all success thresholds. |
...
| Engineering quality | The submission includes a metrics schema, policy card, stress replay, trajectory trace, contact timeline, label CSV, sensor manifest, SRT captions, rubric scorecard, manifest, and a validator. |
| Presentation | `media/demo.mp4` gives the moving sequence; `media/keyframes.png` gives an eight-panel storyboard for quick review; `JUDGE_BRIEF.md` provides the short scoring entry point. |
...
1. `media/keyframes.png` - eight-panel storyboard of the entire task.
2. `media/demo.mp4` - generated 32-second video with phase and metric overlays.
3. `JUDGE_BRIEF.md` - short rubric mapping and numeric evidence.
4. `dataset/metrics.json` - success criteria and closed-loop summary.
5. `dataset/contact_timeline.json` - per-sample fingertip contact, balance, and
   slip evidence.
6. `dataset/stress_eval.json` - fixed-seed perturbation replay.
7. `learned_policy_weights.json` and `dataset/training_report.json` - learned
   policy provenance.
