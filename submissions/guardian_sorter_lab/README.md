...
machine-readable evidence pack. The regenerated demo is now a paced 75-second
run: it keeps the middle of the video deliberately clean with one-line phase
overlays, a dot-based five-finger contact indicator, larger grasp/cap framing,
opening evidence badges, and a closing pass card. The extra time gives the
grasp, uncap, shove/recovery, and care-tool chain more visual breathing room
without adding dense subtitles. The detailed technical evidence lives in the
README, JUDGE_BRIEF, metrics, contact timeline, and stress replay.
...
touch sensors, learned policy weights, contact timelines, a video-to-rubric map,
stress replays, 500Hz control-loop evidence, and a local validator.

Primary judge signal: 11/11 task criteria pass, 96/96 fixed-seed stress rollouts
pass, 214.0 degree cap rotation, 4.0 ms tactile reflex latency, 350 stable
...
| Presentation | `media/demo.mp4` is a paced 75-second demo within the official 1-3 minute window. It uses one-line middle overlays, a dot-based five-finger contact indicator, opening/closing evidence cards, larger motion view, a cap-angle arc, and a 4N/9x disturbance callout. `media/keyframes.png` gives an eight-panel storyboard with concise subtitles, and `JUDGE_BRIEF.md` provides the short scoring entry point. |
...
- Learned policy evidence: 6717 training samples, 1475 validation samples, MAE
  0.022843.
- Learned policy inference: 751 samples in the formal 75-second run.
- Control loop: 500Hz MuJoCo loop with 4.0 ms tactile reflex latency.
- Five-finger stable contact: 350 samples with all five fingers active and
  balanced.
...
- In-hand cap rotation over 200 degrees, five active contacts, 4N lateral shove
  ...
  confirmation.
- Formal pass evidence: 11/11 task criteria, 96/96 fixed-seed stress rollouts,
  100% learned-policy stress success, 500Hz loop, and 4ms tactile reflex latency.
- 75-second paced generated demo video with concise one-line overlays, contact dots,
  opening and closing evidence cards, cap-angle arc, 4N/9x disturbance callout,
  active fingers, cap angle, slip, care-tool states, and policy confidence.
- Generated keyframe storyboard summarizing the 75-second demo in eight readable
...
2. `media/demo.mp4` - generated 75-second video with one-line middle overlays,
   contact dots, opening and closing evidence cards, larger motion view, phase
   labels, cap-angle arc, and 4N/9x disturbance callout.
